...
]);const c=(clients??[]) as any[],p=(projects??[]) as any[],m=(messages??[]) as any[],a=(access??[]) as any[];return <><div className="admin-page-head"><div><div className="kpi">Skylight Reflections Marketing</div><h1>Proposals, Agreements & Projects</h1><p className="muted">Run customizable service sales and delivery from proposal through electronic acceptance, explicit invoicing, project onboarding, milestones, client approvals and recurring-service planning.</p><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}><a className="btn btn-primary" href="/admin/skylight-services">Customize Services</a><a className="btn btn-primary" href="/admin/skylight-intake">Intake & Client Assets</a><a className="btn btn-light" href="/admin/skylight-invoices">Service Invoices</a><a className="btn btn-light" href="/admin/skylight-operations/visibility-retention">Visibility Results & Retention 4.5</a><a className="btn btn-light" href="/admin/skylight-sales">Sales Command Center</a></div></div><span className="badge verified">Admin Only</span></div><SkylightOperationsManager clients={c} services={(services??[]) as any[]} packages={(packages??[]) as any[]} packageItems={(packageItems??[]) as any[]} templates={(templates??[]) as any[]} proposals={(proposals??[]) as any[]} proposalItems={(proposalItems??[]) as any[]} agreements={(agreements??[]) as any[]} projects={p} projectServices={(projectServices??[]) as any[]} milestones={(milestones??[]) as any[]} tasks={(tasks??[]) as any[]} recurring={(recurring??[]) as any[]} messages={m} access={a} invites={(invites??[]) as any[]} invoices={(invoices??[]) as any[]}/><div style={{height:18}}/><SkylightClientCommunicationManager clients={c} projects={p} messages={m} access={a}/></>}
