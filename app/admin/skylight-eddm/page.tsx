...
export default async function Page(){
  await requireAdmin('/admin/skylight-eddm')
  const s=await createClient()
  const[{data:settings},{data:packages},{data:markets},{data:interests},{data:spots},{data:assets},{data:financials},{data:activity}]=await Promise.all([
  ...
  const p=(packages??[]) as any[],nameByKey=new Map(p.map(x=>[String(x.package_key),x]))
  const enriched=((interests??[]) as any[]).map(i=>({...i,package:nameByKey.get(String(i.package_key||''))||null}))
  return <>
    <div className="admin-page-head"><div><div className="kpi">Skylight Reflections Marketing</div><h1>EDDM Production & Community Mailers</h1><p className="muted">Operate co-op and dedicated EDDM from interest through spot sales, artwork, payment, print, USPS drop and completed mail delivery.</p></div><span className="badge verified">Admin Only</span></div>
    <SkylightEddmProductionManager tenantId={TENANT_ID} packages={p} markets={(markets??[]) as any[]} interests={enriched} spots={(spots??[]) as any[]} assets={(assets??[]) as any[]} financials={(financials??[]) as any[]} activity={(activity??[]) as any[]}/>
    <SkylightEddmManager settings={(settings??null) as any} packages={p} markets={(markets??[]) as any[]} interests={enriched}/>
  </>
}
