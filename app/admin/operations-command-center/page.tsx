...
export default async function Page({searchParams}:{searchParams:Promise<Record<string,SearchValue>>}){
  ...
  const [businesses,locations,categories,branches,areas,bc,seo,prospects,sponsors,media,searches,growth]=await Promise.all([
  ...
  const errors=[businesses.error,locations.error,categories.error,branches.error,areas.error,bc.error,seo.error,prospects.error,sponsors.error,media.error,searches.error,growth.error].filter(Boolean)
  ...
  const qualityRows:QualityRow[]=biz.map(b=>{
    ...
    if(!String(b.website||'').trim())issues.push('Missing website')
  ...
  const missingWebsite=qualityRows.filter(r=>r.issues.includes('Missing website')).length

  return <>
    <div className="admin-page-head"><div><div className="kpi">Private Operations Intelligence</div><h1>Growth Operations Command Center</h1><p className="muted">One operating view for SEO eligibility, listing quality, owner acquisition and Sponsored inventory. Internal workflow scores never affect public organic ranking.</p></div><div className="admin-row-actions"><Link className="btn btn-primary" href="/admin/inventory-expansion">Market Coverage</Link><Link className="btn btn-light" href="/admin/launch-growth">Revenue Growth</Link></div></div>
    ...
    <div className="grid grid-4" style={{marginTop:18}}><Link className="admin-card admin-quick-card" href="/admin/inventory-expansion"><strong>Inventory Coverage</strong><p className="small muted">Find 0/1/2-provider gaps and fastest threshold unlocks.</p><span className="kpi">Open →</span></Link><Link className="admin-card admin-quick-card" href="/admin/content-intelligence"><strong>SEO & Content</strong><p className="small muted">Review index eligibility, guides and demand reconciliation.</p><span className="kpi">Open →</span></Link><Link className="admin-card admin-quick-card" href="/admin/growth-opportunities"><strong>Growth Queue</strong><p className="small muted">Assign and work persistent internal opportunities.</p><span className="kpi">Open →</span></Link><Link className="admin-card admin-quick-card" href="/admin/revenue-intelligence"><strong>Revenue Intelligence</strong><p className="small muted">Review measured plan, lead and receivables performance.</p><span className="kpi">Open →</span></Link></div>
  </>
}
