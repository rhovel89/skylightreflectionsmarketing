...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, SearchValue>> }) {
  ...
  const editorRows = shown.map(row => ({
    ...
    contact_path: contactable(row) ? 'Available' : 'Research needed',
  ...
  return <>
    <div className="admin-page-head"><div><div className="kpi">Private Growth Operations</div><h1>Growth Opportunity Workbench</h1><p className="muted">Prioritize claim activation, contact enrichment and separate Skylight marketing opportunities using generated source facts and staff-controlled workflow state. Opportunity scores never affect public organic ranking.</p></div><span className="badge neutral">Internal only</span></div>
    {error && <div className="notice warn">Growth opportunities could not be fully loaded: {error.message}</div>}
    <div className="notice"><strong>Evidence rules:</strong> opportunity rows are signals, not proof of outreach, revenue, ownership or verification. Current records with no evidence-backed monetary estimate remain at $0 instead of being assigned invented revenue values.</div>
    ...
    <div className="admin-card" style={{marginTop:18}}>
      ...
      <form method="get" className="grid grid-3" style={{alignItems:'end'}}>
        ...
        <label className="field"><span>Contact Path</span><select name="contact" defaultValue={contactFilter}><option value="">All</option><option value="ready">Contact path available</option><option value="missing">Contact research needed</option></select></label>
