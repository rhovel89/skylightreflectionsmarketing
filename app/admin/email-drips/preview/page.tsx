...
import { savePreviewDripStep,sendTestDripEmail } from './actions'
...
const editMessage=(value:string)=>value==='saved'?'Email step saved. The production preview below now reflects the new copy.':value==='live'?'Pause this campaign before editing a live email.':value==='required'?'Subject and email body are required.':value==='failed'?'The email step could not be saved.':value==='missing'?'Campaign not found.':value==='invalid'?'Choose a valid campaign and email step.':''
...
export default async function Page({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const editable=Boolean(campaign&&campaign.status!=='live')
  return <>
    <div className="email-preview-head"><div><div className="kpi">Quality Assurance</div><h1>Visual Email Editor, Preview & Test Lab</h1><p className="muted">Edit saved email copy, inspect the same production template your recipients will receive, switch between desktop and mobile, and send a private test before approval.</p></div><a className="btn btn-light" href="/admin/email-drips">← Campaign Studio</a></div>
    {error&&<div className="notice warn">{error.message}</div>}
    {edit&&editMessage(edit)&&<div className={`notice ${edit==='saved'?'success':'warn'}`}><strong>Visual editor:</strong> {editMessage(edit)}</div>}
    {test&&testMessage(test)&&<div className={`notice ${test==='sent'?'success':'warn'}`}><strong>Test send:</strong> {testMessage(test)}</div>}
    {!campaign?<div className="admin-card"><h2>No campaigns yet</h2><p className="muted">Create a campaign in Campaign Studio, then return here to edit and preview it.</p></div>:<div className="email-preview-grid">
      <aside className="email-preview-sidebar">
        <div className="kpi">Saved Campaign</div><h2>{campaign.name}</h2><p className="small muted">Status: {campaign.status}{campaign.status==='approved'||campaign.status==='paused'?' · editing will return this campaign to Draft':''}</p>
        <form method="get" action="/admin/email-drips/preview"><label>Campaign<select name="campaign" defaultValue={campaign.id}>{campaigns.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>Email Step<select name="step" defaultValue={step?.id||''}>{steps.map((x:any)=><option key={x.id} value={x.id}>Step {x.step_order} · Day +{x.delay_days} · {x.subject}</option>)}</select></label><input type="hidden" name="device" value={device}/><button className="btn btn-light">Load Email</button></form>
        {step&&<><div className="preview-device-switch"><a className={device==='desktop'?'active':''} href={`/admin/email-drips/preview?campaign=${campaign.id}&step=${step.id}&device=desktop`}>Desktop</a><a className={device==='mobile'?'active':''} href={`/admin/email-drips/preview?campaign=${campaign.id}&step=${step.id}&device=mobile`}>Mobile</a></div>
        <div className="visual-editor-box"><div className="visual-editor-title"><div><div className="kpi">Visual Editor</div><h3>Edit this email</h3></div><span className={`badge ${editable?'neutral':'sponsored'}`}>{editable?'Editable':'Pause to edit'}</span></div><form action={savePreviewDripStep}><input type="hidden" name="campaign_id" value={campaign.id}/><input type="hidden" name="step_id" value={step.id}/><input type="hidden" name="device" value={device}/><div className="visual-editor-row"><label>Send Day<input type="number" min="0" name="delay_days" defaultValue={step.delay_days} disabled={!editable}/></label><label className="check visual-active-check"><input type="checkbox" name="is_active" defaultChecked={step.is_active} disabled={!editable}/> Active</label></div><label>Subject<input name="subject" maxLength={220} defaultValue={step.subject} required disabled={!editable}/><small>{step.subject.length}/220 characters saved</small></label><label>Preheader<input name="preheader" maxLength={260} defaultValue={step.preheader||''} disabled={!editable} placeholder="Short inbox preview text"/></label><label>Email Body<textarea name="body" maxLength={8000} defaultValue={step.body} required disabled={!editable} style={{minHeight:260}}/></label><div className="visual-editor-row"><label>CTA Label<input name="cta_label" maxLength={120} defaultValue={step.cta_label||''} disabled={!editable}/></label><label>CTA URL<input name="cta_url" maxLength={600} defaultValue={step.cta_url||''} disabled={!editable} placeholder="/for-businesses"/></label></div><button className="btn btn-primary full" disabled={!editable}>Save & Refresh Production Preview</button></form><p className="small muted">Changes to an Approved or Paused campaign automatically return it to Draft for another review. Live campaigns must be paused first.</p></div>
        <div className="preview-meta"><div><b>Rendered Subject</b>{step.subject}</div><div><b>Rendered Preheader</b>{step.preheader||'No preheader set'}</div><div><b>Tracked CTA Destination</b>{cta||'No CTA'}</div></div><div className="preview-checks"><div className={`preview-check ${step.subject.length<=60?'ok':'warn'}`}><strong>Subject length:</strong> {step.subject.length} characters {step.subject.length<=60?'· good for most inboxes':'· consider shortening'}</div><div className={`preview-check ${step.preheader?'ok':'warn'}`}><strong>Preheader:</strong> {step.preheader?'Configured':'Missing'}</div><div className={`preview-check ${postal?'ok':'warn'}`}><strong>Marketing footer:</strong> {postal?'Postal address configured':'Postal address still required before live promotional delivery'}</div><div className={`preview-check ${provider?'ok':'warn'}`}><strong>Test sender:</strong> {provider?'Provider configured':'Provider not configured yet'}</div></div><div className="test-send-box"><strong>Send Test Email to Myself</strong><p className="small muted">Sends only to {adminEmail}. It does not enroll a business, advance a sequence or count toward campaign analytics.</p><form action={sendTestDripEmail}><input type="hidden" name="campaign_id" value={campaign.id}/><input type="hidden" name="step_id" value={step.id}/><input type="hidden" name="device" value={device}/><button className="btn btn-primary" disabled={!provider}>Send Test to Myself</button></form></div></>}
      </aside>
      <section className="email-preview-stage"><div className="section-head compact-head"><div><div className="kpi">Production HTML</div><h2>{device==='mobile'?'Mobile inbox preview':'Desktop inbox preview'}</h2><p className="small muted">This frame uses the exact renderer used for real sends. Save an edit on the left and this preview refreshes from the database immediately.</p></div></div>{step?<div className={`preview-frame-shell ${device}`}><iframe sandbox="" title={`Email preview: ${step.subject}`} srcDoc={html}/></div>:<div className="empty-cell">This campaign has no email steps yet.</div>}</section>
    </div>}
  </>
}
