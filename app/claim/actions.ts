...
export async function submitOwnershipClaim(_:ClaimState,fd:FormData):Promise<ClaimState>{
  try{
    await requireUser(`/claim?business=${encodeURIComponent(text(fd,'business_id'))}`)
    ...
    if(error){const m=error.message;return{ok:false,message:m.includes('claim_invitation_email_mismatch')?'This pending profile was approved for a different email address. Sign in with the email used on the original business submission or contact staff.':m.includes('claim_already_submitted')?'You already have an ownership claim under review for this business.':m.includes('business_already_claimed')?'This business is already claimed.':m}}
