...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [...fixed.map(([path,changeFrequency,priority])=>({url:base+path,changeFrequency,priority})),...(biz??[]).flatMap(x=>[{url:`${base}/business/${x.slug}`,lastModified:x.updated_at||undefined,changeFrequency:'monthly' as const,priority:.7},{url:`${base}/business/${x.slug}/community`,lastModified:x.updated_at||undefined,changeFrequency:'weekly' as const,priority:.62}]),...seoMap.values(),...(guides??[]).map(x=>({url:`${base}/guides/${x.slug}`,lastModified:x.updated_at||undefined,changeFrequency:'monthly' as const,priority:.7})),...estateGuides.map(g=>({url:`${base}/estate-planning/guides/${g.slug}`,lastModified:g.updated,changeFrequency:'monthly' as const,priority:.82}))]
}
