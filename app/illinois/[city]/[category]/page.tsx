...
export async function generateMetadata({ params }: { params: Promise<{ city: string; category: string }> }): Promise<Metadata> {
  ...
  const [seo, eligible] = await Promise.all([getSeoPage(loc.name, cat.name),getBusinesses({ city, category, limit: 3 })]); const title = seo?.title || `${cat.name} in ${loc.name}, IL`; const description = seo?.description || `Compare published ${cat.name.toLowerCase()} profiles serving ${loc.name}, Illinois.`; const canonical = `${getSiteUrl()}/illinois/${city}/${category}`
...
export default async function Page({ params }: { params: Promise<{ city: string; category: string }> }) {
  ...
  const label = categoryLabel(cat.slug, cat.name, cat.vertical),article = indefiniteArticle(label.singular),officialResource = trustResource(cat.vertical),faq = [
    {question:`How many ${label.plural} profiles are published in ${loc.name}?`,answer:`This page currently includes ${businesses.length} published ${label.plural} profile${businesses.length === 1 ? '' : 's'} connected to ${loc.name} through an active physical location or a clearly labeled service area. The count can change as directory inventory is reviewed, added or updated.`},
