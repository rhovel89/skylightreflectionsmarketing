...
export async function POST(req:Request){
 try{
  ...
  const {data:offer,error}=await s.from('lead_marketplace_offers').select('id,lead_id,business_id,price_cents,status,expires_at,checkout_url,access_mode,checkout_enabled,leads(service,city),businesses(name)').eq('tenant_id',TENANT_ID).eq('id',offer_id).maybeSingle()
  ...
  const {data:access,error:accessError}=await s.rpc('get_business_lead_access',{p_business_id:offer.business_id});if(accessError||!(access as any)?.lead_inbox)return NextResponse.json({error:'Lead Inbox access is required before purchasing a lead. Pro includes Lead Inbox; Featured can add it separately.'},{status:403})
  if(offer.access_mode==='admin_request'&&!offer.checkout_enabled)return NextResponse.json({error:'Admin approval is required before checkout for this lead. Request the lead from the marketplace first.'},{status:403})
  ...
  const form=new URLSearchParams();form.set('mode','payment');form.set('success_url',`${SITE_URL}/business-portal/lead-marketplace?business=${encodeURIComponent(offer.business_id)}&purchase=success`);form.set('cancel_url',`${SITE_URL}/business-portal/lead-marketplace?business=${encodeURIComponent(offer.business_id)}&purchase=canceled`);form.set('line_items[0][quantity]','1');form.set('line_items[0][price_data][currency]','usd');form.set('line_items[0][price_data][unit_amount]',String(offer.price_cents));form.set('line_items[0][price_data][product_data][name]',`Central Illinois Local Pros Lead — ${lead?.service||'Local Service'} · ${lead?.city||'Central Illinois'}`);form.set('line_items[0][price_data][product_data][description]',`One-time prepaid lead access for ${business?.name||'your business'}. Consumer contact details are delivered only after successful payment to the enabled Lead Inbox.`);form.set('metadata[app]','central-il-local-pros');form.set('metadata[tenant_id]',TENANT_ID);form.set('metadata[checkout_kind]','lead_purchase');form.set('metadata[lead_offer_id]',offer.id);form.set('metadata[lead_id]',offer.lead_id);form.set('metadata[business_id]',offer.business_id);form.set('payment_intent_data[metadata][app]','central-il-local-pros');form.set('payment_intent_data[metadata][checkout_kind]','lead_purchase');form.set('payment_intent_data[metadata][lead_offer_id]',offer.id);form.set('payment_method_types[0]','card')
