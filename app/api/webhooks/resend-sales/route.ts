...
async function resolveWebhookSecret(s:any){
  const env=process.env.RESEND_WEBHOOK_SECRET?.trim()||''
  if(env)return env
  const q=await s.from('skylight_sales_provider_secrets').select('secret_value').eq('tenant_id',TENANT_ID).eq('provider','resend').eq('secret_key','sales_webhook_signing_secret').maybeSingle()
  if(q.error)throw q.error
  return text(q.data?.secret_value,4000)
}
function verifySvix(raw:string,headers:Headers,secretRaw:string){
  const id=headers.get('svix-id')||'',stamp=headers.get('svix-timestamp')||'',signature=headers.get('svix-signature')||''
  if(!secretRaw||!id||!stamp||!signature)return false
  const ts=Number(stamp)
  if(!Number.isFinite(ts)||Math.abs(Math.floor(Date.now()/1000)-ts)>300)return false
  let secret:Buffer
  try{secret=Buffer.from(secretRaw.startsWith('whsec_')?secretRaw.slice(6):secretRaw,'base64')}catch{return false}
...
function parseAddress(v:any){
  const raw=Array.isArray(v)?text(v[0],500):text(v,500),m=raw.match(/^(.*?)\s*<([^>]+)>$/),email=text(m?.[2]||raw,320).toLowerCase()
...
async function matchConversation(s:any,inReplyTo:string,senderEmail:string){
  if(inReplyTo){
    const a=await s.from('skylight_sales_outreach_drafts').select('id,opportunity_id,prospect_id,campaign_id,campaign_member_id').eq('tenant_id',TENANT_ID).eq('provider_message_header',inReplyTo).order('sent_at',{ascending:false}).limit(1).maybeSingle();if(a.error)throw a.error;if(a.data)return {...a.data,source:'outreach_draft'}
    const b=await s.from('skylight_sales_response_drafts').select('id,opportunity_id,prospect_id,campaign_id,campaign_member_id').eq('tenant_id',TENANT_ID).eq('provider_message_header',inReplyTo).order('sent_at',{ascending:false}).limit(1).maybeSingle();if(b.error)throw b.error;if(b.data)return {...b.data,source:'response_draft'}
  }
  if(senderEmail){
    const p=await s.from('business_prospects').select('id').eq('tenant_id',TENANT_ID).ilike('owner_contact_email',senderEmail).order('updated_at',{ascending:false}).limit(1).maybeSingle();if(p.error)throw p.error
    if(p.data){const o=await s.from('skylight_sales_opportunities').select('id,prospect_id').eq('tenant_id',TENANT_ID).eq('prospect_id',p.data.id).order('active',{ascending:false}).order('updated_at',{ascending:false}).limit(1).maybeSingle();if(o.error)throw o.error;if(o.data){const m=await s.from('skylight_sales_campaign_members').select('id,campaign_id').eq('opportunity_id',o.data.id).order('updated_at',{ascending:false}).limit(1).maybeSingle();if(m.error)throw m.error;return {opportunity_id:o.data.id,prospect_id:p.data.id,campaign_id:m.data?.campaign_id||null,campaign_member_id:m.data?.id||null,id:null,source:'sender_fallback'}}}
  }
  return null
}
...
export async function POST(req:Request){
  ...
  let s:any,secret=''
  try{s=createServiceClient();secret=await resolveWebhookSecret(s)}catch{return NextResponse.json({ok:false,error:'Inbound email server credentials are not configured.'},{status:503})}
  if(!secret)return NextResponse.json({ok:false,error:'Inbound email webhook signing secret is not configured.'},{status:503})
  if(!verifySvix(raw,req.headers,secret))return NextResponse.json({ok:false,error:'Invalid webhook signature.'},{status:401})
  let event:Row
  try{event=JSON.parse(raw)}catch{return NextResponse.json({ok:false,error:'Invalid JSON.'},{status:400})}
  const eventType=text(event.type,100),emailId=text(event?.data?.email_id||event?.data?.id,240)
  ...
  if(receipt.error){if(String(receipt.error.code)==='23505')return NextResponse.json({ok:true,duplicate:true});return NextResponse.json({ok:false,error:'Unable to register webhook receipt.'},{status:500})}
  try{
    if(eventType==='email.sent'){
      ...
      await markReceipt(s,webhookId,{status:'processed'});return NextResponse.json({ok:true,event:eventType})
    }
    if(eventType!=='email.received'){await markReceipt(s,webhookId,{status:'ignored'});return NextResponse.json({ok:true,ignored:true})}
    if(!emailId)throw new Error('Resend inbound event did not include an email id.')
    const apiKey=process.env.RESEND_API_KEY?.trim()||'';if(!apiKey)throw new Error('RESEND_API_KEY is not configured.')
    const r=await fetch(`https://api.resend.com/emails/receiving/${encodeURIComponent(emailId)}`,{headers:{Authorization:`Bearer ${apiKey}`},cache:'no-store'}),full=await r.json().catch(()=>({}));if(!r.ok)throw new Error(text(full?.message,1000)||`Unable to retrieve inbound email (${r.status}).`)
    const headers=headerMap(full.headers),from=parseAddress(full.from||event?.data?.from),senderEmail=from.email||'',subject=text(full.subject||event?.data?.subject,500),messageId=text(full.message_id||headers['message-id'],500),inReplyTo=text(headers['in-reply-to'],1000),references=text(headers['references'],3000)
    const recipients=(Array.isArray(full.to)?full.to:Array.isArray(event?.data?.to)?event.data.to:[]).map((x:any)=>parseAddress(x).email).filter(Boolean),body=text(full.text,24000)||text(stripHtml(text(full.html,50000)),24000)||'[No readable message body supplied]',attachments=Array.isArray(full.attachments)?full.attachments.slice(0,50):[],matched=await matchConversation(s,inReplyTo,senderEmail)
    if(!matched){const q=await s.from('skylight_sales_unmatched_inbound').insert({tenant_id:TENANT_ID,provider:'resend',provider_email_id:emailId,provider_message_id:messageId||null,in_reply_to_header:inReplyTo||null,references_header:references||null,sender_name:from.name,sender_email:senderEmail||null,recipient_emails:recipients,subject:subject||null,body,headers,attachments,received_at:new Date().toISOString(),status:'unmatched'});if(q.error&&String(q.error.code)!=='23505')throw q.error;await markReceipt(s,webhookId,{status:'processed'});return NextResponse.json({ok:true,matched:false,quarantined:true})}
    const sg=suggest(subject,body),receivedAt=new Date().toISOString(),ins=await s.from('skylight_sales_replies').insert({tenant_id:TENANT_ID,opportunity_id:matched.opportunity_id,prospect_id:matched.prospect_id,draft_id:matched.source==='outreach_draft'?matched.id:null,campaign_id:matched.campaign_id||null,campaign_member_id:matched.campaign_member_id||null,channel:'email',provider:'resend',provider_email_id:emailId,provider_message_id:messageId||null,in_reply_to_header:inReplyTo||null,references_header:references||null,sender_name:from.name,sender_email:senderEmail||null,subject:subject||null,body,received_at:receivedAt,classification_suggestion:sg.classification,classification_confidence:sg.confidence,suggestion_reason:sg.reason,review_status:'pending',headers,attachments,metadata:{ingress:'resend_webhook',classification_advisory:true,automatic_stage_change:false}}).select('id').single()
    ...
    const opp=await s.from('skylight_sales_opportunities').select('first_reply_at').eq('tenant_id',TENANT_ID).eq('id',matched.opportunity_id).single();if(opp.error)throw opp.error
    ...
    await markReceipt(s,webhookId,{status:'processed'});return NextResponse.json({ok:true,matched:true,pending_review:true})
  }catch(e:any){await markReceipt(s,webhookId,{status:'failed',error_message:text(e?.message,2000)}).catch(()=>{});return NextResponse.json({ok:false,error:'Inbound email processing failed.'},{status:500})}
}
