...
export async function VerticalPage({
  ...
  const [cats, inventory, locations, guides, featured] = await Promise.all([
    ...
    getLocations(),
    getGuides(120),
    getFeaturedSidebarBusinesses({ pagePath: path, placement: placementByVertical[vertical], limit: 4 }),
  ])

  const usedCategorySlugs = new Set(
    (inventory as any[]).flatMap((business: any) =>
      (business.business_categories ?? []).map((row: any) => row.categories?.slug).filter(Boolean),
    ),
  )
  ...
  const relevant = (guides as any[])
    .filter((guide) => (preferredType && guide.type === preferredType) || (guide.category && categoryNames.has(guide.category)))
    .slice(0, 3)
  ...
  const schema = [
    ...
    ...(businesses.length ? [{
      ...
      itemListElement: businesses.map((business: any, index: number) => ({
        ...
        item: { '@type': 'LocalBusiness', name: business.name, url: `${base}/business/${business.slug}` },
      })),
    }] : []),
  ]
  ...
  return (
    <SiteShell>
      ...
      <main>
        <section className="pagehero"><div className="container"><div className="crumb">Home › {title}</div><h1>{title}</h1><p>{description}</p></div></section>
        <section className="section"><div className="container">
          ...
          <div className="featured-content-layout" style={{ marginTop: 24 }}><div className="featured-content-main">
            <div className="grid grid-4">{visibleCats.map((category) => <Link className="card" key={category.id} href={`/search?category=${category.slug}`}><strong>{category.name}</strong><p className="muted small">Browse local {category.name.toLowerCase()}</p></Link>)}</div>
            {relevant.length > 0 && <section className="inline-guide-section"><div className="section-head compact-head"><div><div className="kpi">Helpful local reading</div><h2>{title} Guides</h2><p className="muted">Practical articles to help you plan, compare and make better-informed local decisions.</p></div><Link href="/guides">All Local Guides →</Link></div><div className="inline-guide-grid">{relevant.map((guide: any) => <Link className="inline-guide-card" key={guide.id} href={`/guides/${guide.slug}`}><span>{guide.city ? `${guide.city}, IL` : guide.type || 'Local Guide'}</span><strong>{guide.title}</strong><p>{guide.summary}</p></Link>)}</div></section>}
            <div className="section-head" style={{ marginTop: 45 }}><div><h2>Published {title}</h2><p className="muted">Listings are displayed from the live directory inventory. Paid Featured inventory is shown separately.</p></div></div>
            <div className="business-list">{businesses.map((business) => <BusinessCard key={business.id} business={business} />)}</div>
            {!businesses.length && <div className="empty">No published listings match this section yet.</div>}
          </div><FeaturedBusinessSidebar businesses={featured as any[]} contextLabel={title.toLowerCase()} /></div>
        </div></section>
      </main>
    </SiteShell>
  )
