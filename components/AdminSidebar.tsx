...
type NavGroup = { id: string; label: string; description: string; items: NavItem[] }

const shortcuts: NavItem[] = [
  { href: '/admin', label: 'Overview' },
  { href: '/admin/action-center', label: 'Today' },
...
const groups: NavGroup[] = [
  {
    id: 'directory', label: 'Directory Operations', description: 'Listings, approvals, claims and customer-facing business data', items: [
  ...
  {
    id: 'seo-content', label: 'SEO & Content', description: 'Markets, categories, content quality and organic eligibility', items: [
  ...
  {
    id: 'revenue', label: 'Revenue & Monetization', description: 'Leads, plans, sponsorships, subscriptions and revenue operations', items: [
  ...
  {
    id: 'site-network', label: 'Site & Network', description: 'Brand, public navigation, site settings and directory replication', items: [
  ...
  {
    id: 'system', label: 'System & Administration', description: 'Readiness, imports, staff access and audit history', items: [
...
export function AdminSidebar() {
  ...
  const visibleGroups = useMemo(() => {
    ...
    return groups.map((group) => ({ ...group, items: group.items.filter((item) => `${item.label} ${item.keywords ?? ''} ${group.label}`.toLowerCase().includes(normalizedQuery)) })).filter((group) => group.items.length > 0)
  }, [normalizedQuery])
  const isActive = (href: string) => { if (href.includes('?')) return false; const path = pathOnly(href); if (path === '/admin') return pathname === '/admin'; return pathname === path || pathname.startsWith(`${path}/`) }
