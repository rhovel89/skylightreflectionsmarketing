...
export function SalesConversionWorkbench({opportunities,replies,meetings,handoffs,conversionEvents,drafts,campaigns,members,proposals}:Props){
  ...
  const campaignMetrics=useMemo(()=>campaigns.map(c=>{
    ...
    return {...c,sent,replies:rs.length,positive:confirmedPositive,meetings:ms,handoffs:hs,wins,losses}
  ...
  return <div style={{display:'grid',gap:24,marginTop:24}}>
    ...
    <section className="admin-card">
      ...
      <form onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);post({action:'book_meeting',opportunity_id:f.get('opportunity_id'),meeting_type:f.get('meeting_type'),scheduled_at:f.get('scheduled_at'),duration_minutes:f.get('duration_minutes'),location_type:f.get('location_type'),location_detail:f.get('location_detail'),notes:f.get('notes')},'book_meeting')}} style={{display:'grid',gap:10,marginBottom:18}}>
