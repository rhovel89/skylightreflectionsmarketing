...
const STATIC_WORKSPACES: Workspace[] = [
  { id:'priorities', label:'Priorities', items:[
  ...
  { id:'businesses', label:'Businesses', items:[
  ...
  { id:'client-work', label:'Client Work', items:[
  ...
  { id:'growth-seo', label:'Growth & SEO', items:[
...
function pathOnly(href:string){return href.split('?')[0]}
...
function getBusinessDetailWorkspace(pathname:string):Workspace|null{
  ...
  return {id:'business-detail',label:'Business Workspace',items:[
...
function chooseWorkspace(pathname:string):Workspace|null{
  const business=getBusinessDetailWorkspace(pathname);if(business)return business
...
export function AdminWorkspaceNav(){
  const pathname=usePathname(),workspace=chooseWorkspace(pathname)
  if(!workspace)return null
  const ranked=workspace.items.filter(item=>!item.href.includes('?')).map(item=>({item,path:pathOnly(item.href)})).filter(({path})=>pathname===path||pathname.startsWith(`${path}/`)).sort((a,b)=>b.path.length-a.path.length)
