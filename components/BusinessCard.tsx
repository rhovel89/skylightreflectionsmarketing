...
export function BusinessCard({business,claimMode=false}:{business:Business;claimMode?:boolean}){
  ...
  const profileLabel=claimMode&&!b.claimed?'Open & Claim':'View Profile'
  return <article className={`business-card public-business-card${claimMode?' claim-mode-card':''}`}>
    ...
    <div className="business-main">
      <div className="badges">{b.verified&&<span className="badge verified">Verified</span>}{b.is_sponsored&&<span className="badge sponsored">Sponsored</span>}{b.claimed&&<span className="badge neutral">Claimed</span>}{serviceAreaOnly&&<span className="badge neutral">Service Area</span>}</div>
      ...
      <p className="business-card-disclosure">Sponsored placement, when shown, is labeled separately and does not control organic directory order.</p>
    </div>
  </article>
}
