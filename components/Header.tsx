...
export function Header({site,navigation=[]}:{site:SiteSettings;navigation?:NavItem[]}){
 ...
 return <header className="site-header"><div className="container nav"><Link href="/" className="brand-link"><BrandLogo site={site} compact/></Link><nav className="navlinks">{links.map(n=><Link key={n.id} href={n.href}>{n.label}</Link>)}</nav><div className="nav-actions"><Link className="btn btn-primary" href="/for-businesses#list">List Your Business</Link><Link className="btn btn-light" href="/account">My Account</Link></div></div></header>
}
