      ...
      return <details className="admin-card" key={lead.id} open={sale?.follow_up_status==='new'}>
        ...
        <form onSubmit={e=>void save(e,lead.id)} style={{display:'grid',gap:12,marginTop:14}}>
          <div className="grid grid-3">
            ...
            <label className="field"><span>Lead sale status</span><select name="sale_status" defaultValue={sale?.sale_status||'not_delivered'}><option value="not_delivered">Not Delivered</option><option value="delivered">Delivered to Estate Legacy Pro</option><option value="sold">Sold</option><option value="not_sold">Not Sold</option><option value="invalid">Invalid</option></select></label>
          ...
          <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}><button className="btn btn-primary" disabled={busy===lead.id}>{busy===lead.id?'Saving…':'Save Lead Update'}</button><a className="btn btn-light" href="https://estatelegacypro.com/" target="_blank" rel="noopener noreferrer">Open Estate Legacy Pro</a>{sale?.sale_amount_cents!=null?<span className="badge verified">Recorded sale: {money(sale.sale_amount_cents)}</span>:null}</div>
          <p className="small muted">Saving a status here does not email Estate Legacy Pro, create an invoice, charge anyone, or automatically deliver the lead. Those remain explicit owner actions outside this status tracker.</p>
        </form>
      </details>
    })}
  </div>
