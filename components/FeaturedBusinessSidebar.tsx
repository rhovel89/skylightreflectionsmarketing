...
export function FeaturedBusinessSidebar({businesses,contextLabel='this page'}:{businesses:FeaturedBusiness[];contextLabel?:string}){
  return <aside className="featured-business-sidebar" aria-label="Sponsored local businesses">
    <div className="featured-sidebar-head">
      <div><div className="kpi">Paid local visibility</div><h2>Featured Businesses</h2><p className="featured-sidebar-explainer">Clearly labeled advertising shown separately from organic directory results.</p></div>
    ...
    {businesses.length>0?<div className="featured-sidebar-list">{businesses.map(b=><article className="featured-sidebar-card" key={b.id}>
      ...
      <div className="featured-sidebar-actions"><Link className="btn btn-primary btn-small" href={`/business/${b.slug}`}>View Business</Link>{b.phone&&<TrackedBusinessLink businessId={b.id} eventType="phone_click" className="btn btn-light btn-small" href={`tel:${b.phone}`}>Call</TrackedBusinessLink>}{b.website&&<TrackedBusinessLink businessId={b.id} eventType="website_click" className="btn btn-light btn-small" href={b.website} target="_blank" rel="noopener noreferrer">Website</TrackedBusinessLink>}</div>
      <p className="featured-sidebar-context">Sponsored placement shown while browsing {contextLabel}.</p>
      <p className="featured-card-disclosure">Paid placement only. Sponsorship does not create verification or control organic ordering.</p>
