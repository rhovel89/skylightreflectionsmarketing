...
export async function getFeaturedSidebarBusinesses(opts:{city?:string;category?:string;pagePath?:string;placement?:string;limit?:number}={}):Promise<any[]>{
  try{
    ...
    const[{data:rows,error:businessError},logos]=await Promise.all([s.from('businesses').select('id,slug,name,abbr,phone,website,description,status,business_categories(categories(name,slug)),locations!businesses_primary_location_id_fkey(name,slug)').eq('tenant_id',TENANT_ID).eq('status','published').in('id',ids),approvedLogoUrls(s,ids)])
    if(businessError)return[]
    const byId=new Map((rows??[]).map((b:any)=>[b.id,b]))
    return chosen.map(p=>{const b:any=byId.get(p.business_id);if(!b)return null;const cats=(b.business_categories??[]).map((x:any)=>x.categories?.name).filter(Boolean);const loc=Array.isArray(b.locations)?b.locations[0]:b.locations;return {...b,logo_url:logos.get(b.id)||null,is_sponsored:true,sponsored_category:cats[0]||null,sponsored_city:loc?.name||null,sponsorship_placement:p.placement,sponsorship_id:p.id}}).filter(Boolean)
  }catch{return[]}
}

export async function getHomepageFeaturedBusinesses(limit=6):Promise<Business[]>{try{const s=await createClient(),d=today();const{data:placements,error}=await s.from('sponsorships').select('business_id,priority,sort_order,created_at').eq('tenant_id',TENANT_ID).eq('placement','homepage_featured').eq('active',true).or(`starts_on.is.null,starts_on.lte.${d}`).or(`ends_on.is.null,ends_on.gte.${d}`).order('priority',{ascending:false}).order('sort_order',{ascending:true}).order('created_at',{ascending:false}).limit(Math.max(1,Math.min(12,limit)));if(error||!placements?.length)return[];const ids=placements.map((x:any)=>x.business_id);const[{data:rows,error:businessError},logos]=await Promise.all([s.from('businesses').select('id,slug,name,abbr,phone,email,website,description,hours,rating,review_count,verified,claimed,profile_score,status,price_range,menu_url,ordering_url,reservation_url,attributes,address_text,source_name,source_url,primary_location_id,business_categories(categories(vertical,slug,name)),locations!businesses_primary_location_id_fkey(name,slug)').eq('tenant_id',TENANT_ID).eq('status','published').in('id',ids),approvedLogoUrls(s,ids)]);if(businessError)return[];const byId=new Map((rows??[]).map((b:any)=>[b.id,b]));return ids.map(id=>byId.get(id)).filter(Boolean).map((b:any)=>Object.assign({},b,{logo_url:logos.get(b.id)||null,is_sponsored:true})) as Business[]}catch{return[]}}
