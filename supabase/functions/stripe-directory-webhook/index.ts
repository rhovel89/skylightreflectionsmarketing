  ...
  try{
    ...
    if(event.type==='checkout.session.completed'){
      ...
      if(subscriptionId&&['verified','featured','pro'].includes(planSlug)){
        ...
        if(business&&p.data){const prior=await db.from('subscriptions').select('id').eq('provider','stripe').eq('provider_subscription_id',subscriptionId).maybeSingle();const row={tenant_id:TENANT_ID,business_id:business.id,plan_id:p.data.id,provider:'stripe',provider_customer_id:customerId,provider_subscription_id:subscriptionId,status:'active',billing_interval:interval,starts_at:new Date().toISOString(),updated_at:new Date().toISOString()};if(prior.data)await db.from('subscriptions').update(row).eq('id',prior.data.id);else await db.from('subscriptions').insert(row);if(planSlug==='featured'||planSlug==='pro'){const existingSponsor=await db.from('sponsorships').select('id').eq('provider','stripe').eq('provider_subscription_id',subscriptionId).eq('placement','homepage_featured').maybeSingle();const sponsor={tenant_id:TENANT_ID,business_id:business.id,placement:'homepage_featured',starts_on:new Date().toISOString().slice(0,10),ends_on:null,active:true,provider:'stripe',provider_subscription_id:subscriptionId,origin:'stripe',priority:planSlug==='pro'?200:150,sort_order:100,rotation_weight:1};if(existingSponsor.data)await db.from('sponsorships').update(sponsor).eq('id',existingSponsor.data.id);else await db.from('sponsorships').insert(sponsor)}}else{await db.from('stripe_webhook_events').update({status:'needs_review',processed_at:new Date().toISOString(),error_message:`Could not uniquely match published business or plan. plan=${planSlug}; business=${businessName||''}; listing=${listingUrl||''}; interval=${interval||''}`}).eq('event_id',event.id);return json({ok:true,needs_review:true})}
      }
    }
    if(event.type.startsWith('customer.subscription.')){const subscriptionId=asId(obj);if(subscriptionId){const status=obj.status||'inactive',periodEnd=unixTs(obj.current_period_end),interval=normalizeInterval(obj?.items?.data?.[0]?.price?.recurring?.interval),update:any={status,current_period_end:periodEnd,ends_at:status==='canceled'?new Date().toISOString():null,updated_at:new Date().toISOString()};if(interval)update.billing_interval=interval;await db.from('subscriptions').update(update).eq('provider','stripe').eq('provider_subscription_id',subscriptionId);await db.from('sponsorships').update({active:ACTIVE_STATUSES.has(status),ends_on:ACTIVE_STATUSES.has(status)?null:new Date().toISOString().slice(0,10),updated_at:new Date().toISOString()}).eq('provider','stripe').eq('provider_subscription_id',subscriptionId)}}
