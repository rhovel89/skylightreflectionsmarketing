...
begin
  ...
  v_growth:=private.sync_growth_outreach_tasks(p_tenant_id);
...
comment on function public.refresh_prospect_research_queue(uuid) is
  'Staff-only Sales 3.3 research queue refresh. Prioritizes evidence-backed contact research tasks and reconciles private sales readiness. It never sends outreach or changes listing verification, Sponsored placement, lead billing/routing or organic ranking.';
...
begin
  ...
  v_pipeline:=private.sync_growth_outreach_tasks(p_tenant_id);
  perform private.refresh_skylight_sales_alerts(p_tenant_id);
...
comment on function public.verify_prospect_owner_contact(uuid,uuid,text,text,text,text,text,timestamptz,text) is
  'Atomically records staff-verified owner/decision-maker contact provenance, resolves research work and moves only the private sales opportunity to Contact Ready. It does not send outreach or authorize billing/routing.';
...
comment on function public.record_prospect_research_attempt(uuid,uuid,text,text,timestamptz) is
  'Records a private staff research attempt without fabricating contact data. Keeps the prospect in Research and schedules a deliberate next review. No outreach is sent.';
