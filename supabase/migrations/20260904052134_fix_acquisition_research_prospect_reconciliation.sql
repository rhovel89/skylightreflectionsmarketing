...
as $$
declare
  v_linked_market integer := 0;
  v_linked_name integer := 0;
...
begin
  ...
  with business_context as (
    select b.id,b.name,coalesce(pl.city,sa.city,'') as city
    from public.businesses b
    left join lateral (
      select coalesce(nullif(bl.city,''),l.name) as city
      from public.business_locations bl left join public.locations l on l.id=bl.location_id
    ...
    left join lateral (
      select l.name as city from public.business_service_areas bsa
      join public.locations l on l.id=bsa.location_id and l.tenant_id=b.tenant_id
  ...
  ), exact_market_matches as (
    select p.id as prospect_id,min(b.id::text)::uuid as business_id
    from public.business_prospects p
    join business_context b on lower(p.business_name)=lower(b.name)
      and lower(coalesce(p.city,''))=lower(coalesce(b.city,''))
  ...
  from exact_market_matches m where p.id=m.prospect_id;
  get diagnostics v_linked_market=row_count;

  with matches as (
    select p.id prospect_id,min(b.id::text)::uuid business_id
    from public.business_prospects p
    join public.businesses b on b.tenant_id=p.tenant_id and b.status='published' and not coalesce(b.claimed,false)
  ...
  get diagnostics v_linked_name=row_count;

  with candidates as (
    select b.id business_id,b.name business_name,b.address_text,b.phone,b.website,b.source_name,b.source_url,b.source_checked_at,
      pc.category,pc.vertical,pl.city physical_city,sa.city service_area_city,coalesce(pl.city,sa.city,'') chosen_city
    from public.businesses b
    left join lateral (
      select c.name category,c.vertical from public.business_categories bc join public.categories c on c.id=bc.category_id
    ...
    left join lateral (
      select coalesce(nullif(bl.city,''),l.name) city from public.business_locations bl left join public.locations l on l.id=bl.location_id
      where bl.business_id=b.id and bl.tenant_id=b.tenant_id and bl.is_active=true
      order by coalesce(bl.is_primary,false) desc,bl.created_at asc limit 1
    ) pl on true
    left join lateral (
      select l.name city from public.business_service_areas bsa join public.locations l on l.id=bsa.location_id and l.tenant_id=b.tenant_id
      where bsa.business_id=b.id and l.is_active=true order by l.name asc limit 1
    ) sa on true
    where b.tenant_id=p_tenant_id and b.status='published' and not coalesce(b.claimed,false)
  ...
  select p_tenant_id,c.business_id,c.business_name,c.vertical,c.category,nullif(c.chosen_city,''),c.address_text,c.phone,c.website,
  ...
  where not exists(
    ...
    where p.tenant_id=p_tenant_id and lower(p.business_name)=lower(c.business_name)
      and lower(coalesce(p.city,''))=lower(coalesce(c.chosen_city,''))
  );
  ...
  return jsonb_build_object('tenant_id',p_tenant_id,'linked_by_market',v_linked_market,'linked_by_unique_name',v_linked_name,
    'inserted_research_records',v_inserted,'remaining_without_linked_research',v_remaining,
    'owner_contacts_created',0,'outreach_events_created',0,'refreshed_at',now());
end;
$$;
