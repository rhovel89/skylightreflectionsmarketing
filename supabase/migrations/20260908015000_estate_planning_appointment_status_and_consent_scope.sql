-- Finalize Estate Planning as a preliminary-qualification + appointment-setting workflow.
-- Expand appointment outcomes and scope attorney sharing to basic booked-appointment information.

alter table public.estate_planning_lead_sales
  drop constraint if exists estate_planning_lead_sales_follow_up_status_check;

alter table public.estate_planning_lead_sales
  add constraint estate_planning_lead_sales_follow_up_status_check
  check (follow_up_status = any (array[
    'new'::text,
    'attempted_contact'::text,
    'contacted'::text,
...
declare
  ...
  v_consent_version constant text := 'estate-planning-appointment-setting-v5-2026-09';
  ...
  v_appointment_consent constant text := 'I understand Skylight Reflections Marketing uses my answers for preliminary qualification and appointment coordination. If an appointment is booked, I agree that Skylight may provide the estate-planning attorney or law office with the basic contact and appointment information needed to schedule and follow up on that consultation.';
begin
  if not coalesce(p_consent_to_contact,false) or not coalesce(p_consent_to_share,false) then raise exception 'contact and appointment coordination consent are required'; end if;
  ...
  values(
    ...
    jsonb_build_object(
      ...
      'qualifying_answers_used_by','skylight_appointment_setting'
    )
  );

  insert into public.estate_planning_lead_sales(tenant_id,lead_id,created_by,updated_by)
  values(v_tenant,v_id,v_user,v_user);

  insert into public.estate_planning_lead_referrals(tenant_id,lead_id,created_by,updated_by)
  values(v_tenant,v_id,v_user,v_user);

  insert into public.estate_planning_lead_consents(
    tenant_id,lead_id,consent_version,contact_consent_text,referral_consent_text,source_page,
    landing_path,referrer,utm_source,utm_medium,utm_campaign,utm_content,utm_term,contact_snapshot,recorded_at
  )
  values(
    ...
    jsonb_build_object(
      'consumer_name',trim(p_consumer_name),
      'phone',trim(p_phone),
      'email',trim(p_email),
      'city',trim(p_city),
      'state',v_state,
