-- Estate Planning 1.2 assignment invariants.
-- State capacity counts only that state; reassignment requires unassigned/declined; preparation requires pending review.
...
declare
  ...
  v_ref public.estate_planning_lead_referrals%rowtype;
  v_provider public.estate_planning_providers%rowtype;
  v_state text; v_state_capacity integer; v_open_count integer; v_state_open_count integer; v_event_id uuid;
begin
  ...
  if v_ref.referral_status not in ('unassigned','declined') then raise exception 'current referral state must be unassigned or declined before a new provider can be assigned'; end if;
  ...
  select count(*) into v_state_open_count from public.estate_planning_lead_referrals r where r.tenant_id=v_tenant and r.provider_id=p_provider_id and r.provider_state=v_state and r.lead_id<>p_lead_id and r.referral_status in ('pending_review','approved_for_referral','referred','accepted');
  ...
  if v_state_capacity is not null and v_state_open_count >= v_state_capacity then raise exception 'provider configured state referral capacity is currently full'; end if;
  update public.estate_planning_lead_referrals set provider_id=p_provider_id,provider_name=v_provider.display_name,provider_state=v_state,provider_reference=coalesce(v_provider.external_reference,provider_reference),referral_status='pending_review',assigned_at=now(),referred_at=null,provider_responded_at=null,referral_notes=coalesce(nullif(trim(p_notes),''),referral_notes),updated_by=v_user,updated_at=now() where id=v_ref.id;
...
begin
  ...
  if v_ref.referral_status <> 'pending_review' then raise exception 'handoff preparation requires a pending-review assignment'; end if;
