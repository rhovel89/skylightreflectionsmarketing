  ...
  if new.lead_program_interest is distinct from old.lead_program_interest
    ...
    if new.lead_program_interest = 'interested' then
      ...
      select distinct ur.user_id,
        new.tenant_id,
        'Lead buyer wants more leads'::text,
        (coalesce(v_business_name, 'A claimed business') || ' explicitly asked for more leads after a delivered lead. Review the Lead Buyer CRM before activating any agreement. No billing or future routing was activated by this response.')::text,
        '/admin/lead-buyers'::text,
        v_event_key,
        null::timestamptz,
        now()
