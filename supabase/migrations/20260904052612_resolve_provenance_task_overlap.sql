...
begin
  ...
  set status='cancelled',completed_at=coalesce(completed_at,now()),
      notes=case when nullif(trim(coalesce(t.notes,'')),'') is null
        then 'Closed because contact/provenance research is still required and an open research task already exists.'
        else t.notes||E'\nClosed because contact/provenance research is still required and an open research task already exists.' end
  from public.business_prospects p
  where t.tenant_id=p_tenant_id and t.prospect_id=p.id
    and t.task_type in('claim_invite','marketing_outreach') and t.status in('open','in_progress')
    and ((t.task_type='claim_invite' and p.claim_invite_sent_at is null) or (t.task_type='marketing_outreach' and p.marketing_pitch_sent_at is null))
    ...
    and exists(select 1 from public.outreach_tasks r where r.tenant_id=p_tenant_id and r.prospect_id=p.id and r.task_type='contact_research' and r.status in('open','in_progress') and r.id<>t.id);
  get diagnostics v_redundant_closed=row_count;
  ...
  where t.tenant_id=p_tenant_id and t.prospect_id=p.id and t.task_type='claim_invite' and t.status in('open','in_progress')
    ...
    and not ((coalesce(p.owner_contact_email,'')<>'' or coalesce(p.owner_contact_phone,'')<>'') and coalesce(p.owner_contact_source_url,'')<>'' and p.owner_contact_checked_at is not null)
    and not exists(select 1 from public.outreach_tasks r where r.tenant_id=p_tenant_id and r.prospect_id=p.id and r.task_type='contact_research' and r.status in('open','in_progress') and r.id<>t.id);
...
comment on function private.sync_growth_outreach_tasks(uuid) is
  'Synchronizes staff outreach work while requiring an actionable owner/decision-maker contact plus provenance source URL and checked date before claim or marketing outreach tasks are created. Redundant premature outreach tasks are cancelled when research work already exists. Tasks never imply outreach was sent.';