...
begin
  with demand as (
    select
      ...
      trim(l.city) as city,
      lower(regexp_replace(trim(l.service),'[^a-z0-9]+','','g')) as service_key,
      lower(regexp_replace(trim(l.city),'[^a-z0-9]+','','g')) as city_key,
    ...
    group by
      ...
      lower(regexp_replace(trim(l.service),'[^a-z0-9]+','','g')),
      lower(regexp_replace(trim(l.city),'[^a-z0-9]+','','g'))
    having count(*)>=2
  ), coverage_raw as (
    select distinct
      b.id as business_id,
      lower(regexp_replace(trim(c.name),'[^a-z0-9]+','','g')) as service_key,
      lower(regexp_replace(trim(bl.city),'[^a-z0-9]+','','g')) as city_key,
    ...
    select distinct
      b.id as business_id,
      lower(regexp_replace(trim(c.name),'[^a-z0-9]+','','g')) as service_key,
      lower(regexp_replace(trim(l.name),'[^a-z0-9]+','','g')) as city_key,
    ...
    where not exists (
      ...
      and not (
        li.lead_program_interest='not_interested'
    ...
    select
      ...
      case
        when c.desired_status='snoozed' then c.follow_up_at
        else v_now
      end,
      c.assigned_user_id,
      jsonb_build_object(
...
begin
  ...
  return jsonb_build_object(
    ...
    'lead_buyer_recruitment',v_recruitment,
    'outreach_tasks',v_tasks
  );
end;
$$;
revoke all on function public.refresh_growth_opportunities(uuid) from public,anon;
grant execute on function public.refresh_growth_opportunities(uuid) to authenticated;
...
declare
  ...
  v_recruitment jsonb;
begin
  if not private.has_tenant_role(p_tenant_id,array['admin','super_admin']) then
    raise exception 'Admin access is required.';
  end if;
  v_skylight := private.refresh_skylight_sales_opportunities(p_tenant_id);
  v_recruitment := private.sync_lead_buyer_recruitment_opportunities(p_tenant_id);
  return jsonb_build_object('skylight_sales',v_skylight,'lead_buyer_recruitment',v_recruitment,'refreshed_at',now());
end;
$$;
...
comment on function private.sync_lead_buyer_recruitment_opportunities(uuid) is 'Builds private Lead Buyer recruitment opportunities from 90-day first-party demand and legitimate provider coverage. It never sends outreach, releases consumer contact data, creates/activates agreements, routes leads, bills businesses, changes verification, changes Sponsored status, or changes organic ranking.';
